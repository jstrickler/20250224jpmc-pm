...
with sqlite3.connect("../DATA/presidents.db") as s3conn:
    ...
    terms = [[t] for t in TERMS_TO_UPDATE]
    s3cursor.executemany(PARTY_UPDATE, terms)  # second argument to execute() is iterable of values to fill in placeholders from left to right
